 ...
 *
 *             RNL 09/21/21: Updated code to adhere to all good coding conventions,
 *             such as commenting ALL logic paths.
 */
...
public class CS12Date {
    ...
    public void setMonth(int month) {
        // check for invalid month outside [1-12]
        if ((month < 1) || (month > 12)) {
    ...
    public void setDay(int day) {
        ...
        // check for invalid # of days, which is month-dependent
        if ((day < 1) || (day > maxDays)) {
            error("day must be 1-" + maxDays + ", not changed");                
        }
        
        // # of days is valid for indicated month
        else {
            this.day = day;
        }
    }
    ...
    public int getDateNum() {
        ...
        // sum up all days in all months PRIOR to current one
        for (int i=1; i<month; i++) {
            dateNum += daysInMonth(i, this.year);
        }
        
        // now add on days in current month also
        dateNum += day;
    ...
    public void setDate(int dateNum, int year) {
        ...
        // can't have negative or 0 date numbers
        if (dateNum < 1) {
            error("date number must be >= 0, date unchanged");
            return;
        }
        
        // can't have more than 365 days in a non-leap year (usually the case)
        else if ((!temp.isLeapYear(year)) && (dateNum > 365)) {
            error("date number must be <= 365 for a non-leap year, date unchanged");
            return;
        }
        
        // can't have more than 366 days in a leap year
    ...
    public void laterDate(int numDays) {
        ...
        // if a negative number, regress the date by that many days
        if (numDays < 0) {
            priorDate(-numDays);
        }
        
        // otherwise, safe to advance the date
        else {
        
            // count how many days have been advanced
            while (temp <= numDays) {
                day++;
                temp++;
    ...
    public void priorDate(int numDays) {
        ...
        // if a negative number, advance the date by that many days
        if (numDays < 0) {
            laterDate(-numDays);
        }
        
        // otherwise, safe to regress the date
        else {
    ...
    public boolean equals(Object obj) {
        // check that it's actually a CS12Date object
    ...
    private boolean isValidDate(int mm, int dd, int yyyy) {
        ...
        // year must be 0 or positive
        else if (yyyy < 1) {
            return false;
        }
        // month must be positive AND year-appropriate
    ...
    private void spacer(char ch, int num) {
        // print specified # of specified character
        for (int i=0; i<num; i++) {
            System.out.print(ch);
        }
        System.out.println();
